...
- [x] **Stale `1.2.3.4/24` placeholder in OpenDKIM trustedhosts** — `roles/mail-opendkim/files/trustedhosts`: replaced with `127.0.0.1` + `::1` and a comment explaining the trust scope. Also removed the `10.1.0.0/16` cruft (not the homelab range, origin unknown). Deployed; opendkim restarted cleanly.
...
- [ ] **DKIM private key has no off-VPS backup** — vault-restore plumbing is in place: `roles/mail-opendkim/tasks/main.yml` will write `mail_dkim_private_keys[<domain>]` to disk before `opendkim-genkey` runs, gated by `force: false` so it's idempotent. **Bootstrap still pending**: paste the two live `.private` files into `vault_dkim_private_keys` via `make vault-edit`, then re-run `make check-mail` to confirm no drift. Until then the keys still exist only on the VPS.
